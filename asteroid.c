...
void init_asteroid(Asteroid *a, Vector2 position){
	...
	Vector2 init_vel = {((float)rand()/RAND_MAX - 0.5f) * 150.0f, ((float)rand()/RAND_MAX - 0.5f) * 150.0f};
	a->vel = init_vel;
...
void update_asteroid(Asteroid *a, float dt){
    ...
    // wrap around screen
    	if (a->pos.x < -r)      a->pos.x = w + r;
    	if (a->pos.x > w + r)   a->pos.x = -r;
    	if (a->pos.y < -r)      a->pos.y = h + r;
    	if (a->pos.y > h + r)   a->pos.y = -r;
}
